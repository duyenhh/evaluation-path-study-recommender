...
public class MinCostMaxFlow {
	...
	public static void run(double a, double b, double c, faculty CNTT, faculty CNTT_CLC)
		...
		for (int i = 1; i <= studentNum; i++) {
			...
			student stu;
			if (i < 12)
				stu = new student(1, CNTT, "#" + i + ".csv", AVG[i - 1], target, interest);
			else
				stu = new student(1, CNTT_CLC, "#" + i + ".csv", AVG[i - 1], target, interest);
			// s1.get_score_CF(1);
...
// print out result ----------------------------------------------------------------------
		System.out.println("SET 1 : "+ SUM1 / 19);
		System.out.println("SET 2 : "+ SUM2 / 19);
	...
	public static void main(String[] args) throws IOException, CloneNotSupportedException {
		 ...
		 * //s1.get_score_CF(1); MinCostMaxFlow mcmf = new MinCostMaxFlow();
		 * mcmf.getMaxFlow(stu); SimilarityCompare eva = new
		 * SimilarityCompare(); eva.Caculation(10);
		 */
...
//		run(0.495, 0.01, 0.495, CNTT, CNTT_CLC);
		 ...
		 * )---------------------------------------// int studentNum = 11;
		 * for(int i = 1; i <= studentNum ; i++) { double [] interest = new
		 * double[4]; String []inter_ = inter.nextLine().split(","); int target
		 * = Integer.parseInt(inter_[1]); for(int j = 0; j<4;j++) { interest[j]
		 * = Double.parseDouble(inter_[j+2]); }student stu = new
		 * student(1,KHMT,"#"+i+".csv",AVG[i-1],target,interest);
		 * //s1.get_score_CF(1); stu.get_cost_combine(0.25, 0.25, 0.25, 0.25);
		 * MinCostMaxFlow mcmf = new MinCostMaxFlow(); mcmf.getMaxFlow(stu); }
		 * double SUM = 0.0; for(int i = 1; i <=studentNum ; i++) {
		 * SimilarityCompare eva = new SimilarityCompare();
		 * SUM+=eva.Caculation(i); } System.out.println(SUM/(double)studentNum);
		 * // -------------------------for
		 * eva---------------------------------------//
		 */
		/*
		 * int i =2; student stu = new
		 * student(1,KHMT,"#"+i+".csv",AVG[i-1],target,interest);
		 * //s1.get_score_CF(1); MinCostMaxFlow mcmf = new MinCostMaxFlow();
		 * mcmf.getMaxFlow(stu); SimilarityCompare eva = new
		 * SimilarityCompare(); System.out.println(eva.Caculation(i));
		 */
	}
